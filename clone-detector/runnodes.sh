...
do
    java -Dproperties.rootDir="$rootPATH/" -Dproperties.location="$rootPATH/sourcerer-cc.properties" -Dlog4j.configurationFile="$rootPATH/res/log4j2.xml" -Xms6g -Xmx6g -XX:+UseCompressedOops -jar "$rootPATH/dist/indexbased.SearchManager.jar" $mode $threshold &
    PIDS+="$! "
done
echo $PIDS
